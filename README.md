...
LLM Q&A finetuning dataset — 1149 races, 25,784 results, 5816 QA pairs.
...
- **Wikipedia** (`wikipedia-api`) — 881 drivers, 76 seasons, 16 incidents, 11 constructors + 30 paddock persons + 38 F1 knowledge
...
data/final/qa.jsonl                    5816 pairs {question,answer,source,entity_type,season}
```
QA: `3447` race_result · `1748` driver_bio · `364` champion · `83` person · `63` f1_knowledge · `80` season · `31` incident

## QA schema
`question` paraphrased (3x per race), `answer` factual, `source` traceable `Jolpica:season/round/results` or `Wikipedia:Page`, `entity_type` race_result|season_champion|driver_bio|person|f1_knowledge|incident, `season` int.
...
flowchart TB
    ...
    subgraph PIPELINE["Pipeline"]
        ...
        F["<b>03_merge_qa.py</b><br/>entity-linked"]
    end

    subgraph OUTPUT["Dataset"]
        G["<b>races_normalized.jsonl</b><br/>11 MB"]
        H["<b>qa.jsonl</b><br/>5816 pairs · 2.3 MB"]
...
ds = load_dataset("json", data_files="data/final/qa.jsonl")  # 5816
```
...
python pipeline/05_add_paddock_knowledge.py  # 30 persons + 38 knowledge → +146 QA
python pipeline/03_merge_qa.py            # 5816
```

## Coverage
1950-2025 **100% races (1149/1149)**, 76/76 seasons, 881/881 drivers, 30/30 paddock (Horner now 3 QAs), 38/46 knowledge. FastF1 cross-check 2018-2025 deferred (results only, no telemetry/laps/weather).
